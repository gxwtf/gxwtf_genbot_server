...
def apply_move(state, move):
    ...
    if move is not None:
        ...
        if (tg[ay, ax] == pi
            ...
            if (((by, bx) in state['cities'] and tg[by, bx] == EMPTY)
...
def start_game(client):
    ...
    #client = kong.Generals(userid, username, mode, gameid,)
    numcities2 = 0
    prevdarmies = 0
    for state in client.get_updates():
        ...
        # if state['complete']:
        #     print(["lose", "win"][state['result']],state['replay_url'])
        #     break
        ...
        moves = sorted(moves, key=lambda ay_ax_by_bx: -ag[ay_ax_by_bx[0], ay_ax_by_bx[1]])[:200]
        #print((tg == pi) * ag)
        ...
        # resign
        if (turn > 1000 and len(i for l in state['lands'] if l > 0) == 2
        ...
        # opening (turns 0 - 25)
            ...
            if ply in launch:
                ...
                gen1moves = get_moves(state, gen1[0], gen1[1])
